...
class Main():
    def __init__(self, train_config, env_config, debug=False):
        ...
        feature_map = get_feature_map(dataset) # 返回特征的列表
        fc_struc = get_fc_graph_struc(dataset) # 返回字典代表有向图（没有自己）

        set_device(env_config['device'])
        self.device = get_device()

        fc_edge_index = build_loc_net(fc_struc, list(train.columns), feature_map=feature_map) # 返回边的索引
        ...
        cfg = {
            ...
            'slide_stride': train_config['slide_stride'], # 滑动窗口每次走几步
        }

        train_dataset = TimeDataset(train_dataset_indata, fc_edge_index, mode='train', config=cfg)
         # x:torch.Size([310, 27, 15]) y:torch.Size([310, 27])当前窗口下一个值
        test_dataset = TimeDataset(test_dataset_indata, fc_edge_index, mode='test', config=cfg)
         # x:torch.Size([2034, 27, 15])每次滑动一步 y:torch.Size([2034, 27])
...
if __name__ == "__main__":
    ...
    parser.add_argument('-epoch', help='train epoch', type = int, default=300)
